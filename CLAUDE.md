...
- **URL pattern**:
  `https://dpi.wi.gov/sites/default/files/wise/downloads/enrollment_by_gradelevel_certified_{year}.zip`
- **Format**: ZIP containing CSV file
- **Years available**: 2005-06 through 2023-24 (school year end
  2006-2024)
- **Data structure**: Long format with one row per school/grade/subgroup
  combination
- **Key columns**: DISTRICT_CODE, SCHOOL_CODE, GRADE_LEVEL, GROUP_BY,
  GROUP_BY_VALUE, STUDENT_COUNT

### Published PEM Files (1997-2005) - LEGACY SOURCE

- **URL pattern**:
  `https://dpi.wi.gov/sites/default/files/imce/cst/xls/pem{YY}.xls`
...
- **Note**: Since WISEdash is available for 2006+, we only use PEM files
  for 1997-2005
...
warnings - \[ \] `pytest tests/test_pywischooldata.py` — all tests
pass - \[ \]
