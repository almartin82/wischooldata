...
def get_test_years():
    ...
    if _available_years is None:
        ...
        _available_years = wi.get_available_years()
...
class TestImport:
    ...
    def test_import_functions(self):
        ...
        import pywischooldata as wi
        assert hasattr(wi, 'fetch_enr')
        assert hasattr(wi, 'fetch_enr_multi')
        assert hasattr(wi, 'tidy_enr')
        assert hasattr(wi, 'get_available_years')

    def test_version_exists(self):
        """Package has a version string."""
        import pywischooldata as wi
        assert hasattr(wi, '__version__')
        assert isinstance(wi.__version__, str)
...
class TestFetchEnr:
    ...
    def test_returns_dataframe(self):
        ...
        df = wi.fetch_enr(years['max_year'])
    ...
    def test_total_enrollment_reasonable(self):
        ...
        years = get_test_years()
        df = wi.fetch_enr(years['max_year'])
        # Total enrollment in Wisconsin varies by grade level aggregation
        # Just check that total is positive and reasonable
        total = df['n_students'].sum()
        # Wisconsin should have substantial enrollment
...
class TestFetchEnrMulti:
    ...
    def test_returns_dataframe(self):
        ...
        df = wi.fetch_enr_multi([years['max_year']])
        assert isinstance(df, pd.DataFrame)
    ...
    def test_contains_requested_year(self):
        ...
        df = wi.fetch_enr_multi([test_year])
    ...
    def test_multi_matches_single(self):
        ...
        import pywischooldata as wi
        years = get_test_years()
        df_single = wi.fetch_enr(years['max_year'])
        df_multi = wi.fetch_enr_multi([years['max_year']])
