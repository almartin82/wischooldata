...
test_that("Wisconsin DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Wisconsin DOE URL and verify HTTP 200")
})
...
test_that("Can download Wisconsin enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Wisconsin enrollment file", {
  skip_if_offline()
...
test_that("wischooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- wischooldata::get_available_years()
  ...
  tryCatch({
    raw <- wischooldata:::get_raw_enr(test_year)
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- wischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- wischooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- wischooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- wischooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- wischooldata:::get_cache_path(2024, "enrollment")
